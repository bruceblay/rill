...
class Engine {
  ...
  void generate() {
    ...
    // A recurring answer gesture changes register on two of every four repeats.
    // Derive its placement without consuming score or performance random draws.
    registerCell = 1 + ((rng >> 8) % (phraseLength / 4 - 1));
    registerPhase = (rng >> 16) % 4;
  ...
  void score() {
    ...
    if (degree >= 0) {
      ...
      if ((registerCycle == 1 || registerCycle == 2) &&
        ...
        int anchor = foldPitch(scaleNote(root + voicing[anchorDegree], 1),67,88);
 ...
 public:
  ...
  float sample() {
    ...
    if ((clock & 127) == 0) {
      ...
      feedbackTarget=feedback+feedbackDepth*slow;
