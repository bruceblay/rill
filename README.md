...
A pocket-sized generative instrument for the **M5Stack StickS3**. Still composes delicate, evolving music and animated artwork entirely on the device. Tap for a new piece. Shake for a new visual world.

![Six procedural visual families rendered by Still](docs/images/visuals.png)

*Actual renderer output at the device's 240 × 135 resolution, enlarged for this preview.*

Still is an experimental instrument, currently at **Study 16**. It needs no Wi-Fi, account, audio files, or cloud service. Its sound has been developed around the StickS3's small built-in speaker.
...
build/render build/still.wav 60 42 2
...
Brian Eno's generative work, Cyma Forma's RND synth, and Zach Lieberman's daily sketches helped inform the direction. Still is an independent project, with no affiliation or endorsement implied. The previews are generated by Still; no artwork by those artists is bundled.

Still follows its parent project Pocket Radio's **GPL-3.0-or-later** license. See [LICENSE](LICENSE). Third-party components retain their own licenses; see [dependency notices](docs/DEPENDENCIES.md).
